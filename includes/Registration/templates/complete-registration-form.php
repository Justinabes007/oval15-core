<?php
/**
 * Expects: $u (WP_User), $data (prefill array), $countries (array), $positions (array), $order (\WC_Order|null)
 */
...
<form class="oval15-complete-registration" method="post" enctype="multipart/form-data" style="margin-bottom:20px;">
    ...
    <?php elseif (!empty($_GET['order']) && !empty($_GET['key'])): ?>
    ...
    <p>
        <select class="input-text" name="main-position" id="main-position">
            ...
            <?php foreach ($positions as $p): ?>
                <option value="<?php echo esc_attr($p); ?>" <?php selected($data['main-position'],$p); ?>><?php echo esc_html($p); ?></option>
            <?php endforeach; ?>
        </select>
    </p>
    ...
    <div class="form-group">
        ...
        <p><input type="file" class="input-text" id="v_upload_id" name="v_upload_id" accept="video/*"></p>
    </div>
    ...
    <p style="margin-top:20px;">
        <button type="submit" class="button button-primary">Complete Registration</button>
    </p>
</form>